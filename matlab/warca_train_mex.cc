...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if(kernel_type != PRE_COMP && kernel_type != LIN) {
    throw warca_error("The matlab interface currently supports only precomputed kernels of linear warca\n");
  }
  WARCA<double> warca_object(seed, quiet);
  warca_object.train(K, y, n, n, r, lambda, eta, max_iter, batch_size, max_sampling,  kernel_type, 0, 3, 0);
