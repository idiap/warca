...
int main(int argc, char* argv[]) {
  ...
  parse_command_line<float>(argc, argv, feature_filename, label_filename,  model_filename, param);
  uint32_t num_data, dim_data;
  float* data = NULL;
  uint32_t* labels = NULL;
  read_problem<float>(feature_filename, label_filename, &data, &labels, num_data, dim_data);
