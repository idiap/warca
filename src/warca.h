...
public:
  ...
  ~SimilarPairs() { delete[] pairs_; }
};

/*
  Currently supported kernel types. LIN and LINK are both linear kernels. LIN
  indicates that the model should be trained in the input space and LINK
  indicates that the model should be trained in the kernel space
*/
enum KernelType { LIN = 0, LINK, RBF, POLYNOMIAL, CHI2RBF, PRE_COMP };

template <typename scalar_t> class WARCA {
  KernelType kernel_type_;
  Prand *rnd_;
  // Parameter matrix
  scalar_t *weight_;
  // Support vectors (training points), valid only for non-linear kernels. For
  // linear it will be null.
  scalar_t *SVs_;
  // The number of columns of W, for kernel WARCA it will be the number of
  // training points and for linear warca it is the data dimension;
  uint32_t dim_;
  // The number of rows of W, indicates the dimension of low rank projection;
  uint32_t rank_;
  // The number of support vectors, valid only for non-linear kernels (For
  // kernel warca num_svs_ and dim_ will be same)
  uint32_t num_svs_;
  ...
  void compute_chi2rbf_kernel(const scalar_t *x_matrix,
                              const scalar_t *y_matrix, scalar_t *kernel_matrix,
                              uint32_t num_x, uint32_t num_y, uint32_t dim);
  void compute_chi2rbf_kernel(const scalar_t *x_matrix, scalar_t *kernel_matrix,
                              uint32_t num_x, uint32_t dim);
  void compute_polynomial_kernel(const scalar_t *x_matrix,
                                 const scalar_t *y_matrix,
                                 scalar_t *kernel_matrix, uint32_t num_x,
                                 uint32_t num_y, uint32_t dim);
  void compute_polynomial_kernel(const scalar_t *x_matrix,
                                 scalar_t *kernel_matrix, uint32_t num_x,
                                 uint32_t dim);
  void compute_rbf_kernel(const scalar_t *x_matrix, const scalar_t *y_matrix,
                          scalar_t *kernel_matrix, uint32_t num_x,
                          uint32_t num_y, uint32_t dim);
  void compute_rbf_kernel(const scalar_t *x_matrix, scalar_t *kernel_matrix,
                          uint32_t num_x, uint32_t dim);
  scalar_t l2dist(const scalar_t *x, const scalar_t *y, uint32_t dim);
  scalar_t warp_gradient_dual(const scalar_t *M, const uint32_t *labels,
                              scalar_t *G, uint32_t batch_size,
                              uint32_t max_sampling, SimilarPairs *pairs);
  void train_adam_dual(const scalar_t *kernel_matrix, const uint32_t *labels,
...
public:
  ...
  void train(const scalar_t *data, const uint32_t *labels, uint32_t num_data,
             ...
             KernelType kernel_type, scalar_t gamma, uint32_t degree,
             scalar_t coeff);
  void predict(const scalar_t *data, scalar_t *data_proj, uint32_t num_data,
               uint32_t dim_data);
