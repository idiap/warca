...
template <typename scalar_t> struct WARCATrainParameters {
  uint32_t rank;
...
void exit_warca_train_with_help() {
  throw warca_error(
      ...
      "-m max_sampling : Maximum number of sampling in WARP loss (default "
      "512)\n"
...
void exit_warca_predict_with_help() {
  throw warca_error(
      "Usage: warca_predict feature_file model_file result_file\n");
  exit(1);
}
...
void parse_command_line(uint32_t argc, char **argv,
                        ...
                        WARCATrainParameters<scalar_t> &param) {
  // default values
  uint32_t i;
  param.rank = 2;
...
template <typename scalar_t>
void read_problem(const std::string feature_filename,
                  const std::string label_filename, scalar_t **data,
                  uint32_t **labels, uint32_t &num_data, uint32_t &dim_data) {
  num_data = 0;
  dim_data = 0;
...
template <typename scalar_t>
void read_features(const std::string feature_filename, scalar_t **data,
...
template <typename scalar_t>
void write_features(const std::string feature_filename, const scalar_t *data,
                    uint32_t num_data, uint32_t dim_data) {
  std::ofstream fout(feature_filename.c_str());
  for (uint32_t i = 0; i < num_data; i++) {
    const scalar_t *data_i = data + i * dim_data;
    for (uint32_t j = 0; j < dim_data - 1; j++) {
      fout << data_i[j] << ",";
    }
