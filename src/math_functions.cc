...
template <>
void cblas_gemv<float>(const CBLAS_TRANSPOSE TransA, const int M,
...
template <>
void cblas_gemv<double>(const CBLAS_TRANSPOSE TransA, const int M,
...
template <>
void cblas_copy<float>(const int N, const float* X, float* Y) {
  cblas_scopy(N, X, 1, Y, 1);
}

template <>
void cblas_copy<double>(const int N, const double* X, double* Y) {
...
template<>
void  cblas_exp<double>(const int N, const double *X, double *Y) {
...
template<>
void  cblas_exp<float>(const int N, const float *X, float *Y) {
#pragma omp parallel num_threads(4)
      #pragma omp for
  for(int i = 0; i < N; ++i)
    Y[i] = exp(X[i]);
...
template<>
void  cblas_diag<double>(const int N, const double *X, double *Y) {
#pragma omp parallel num_threads(4)
      #pragma omp for
  for(int i = 0; i < N; ++i)
    Y[i] = X[i + i * N];
...
template<>
void  cblas_diag<float>(const int N, const float *X, float *Y) {
#pragma omp parallel num_threads(4)
      #pragma omp for
  for(int i = 0; i < N; ++i)
    Y[i] = X[i + i * N];
...
template<>
void  cblas_fill<float>(const int N, const float alpha, float *Y) {
#pragma omp parallel num_threads(4)
      #pragma omp for
  for(int i = 0; i < N; ++i)
    Y[i] = alpha;
...
template<>
void  cblas_log<double>(const int N, const double *X, double *Y) {
#pragma omp parallel num_threads(4)
      #pragma omp for
  for(int i = 0; i < N; ++i)
    Y[i] = log(X[i]);
...
template<>
void  cblas_log<float>(const int N, const float *X, float *Y) {
#pragma omp parallel num_threads(4)
      #pragma omp for
  for(int i = 0; i < N; ++i)
    Y[i] = log(X[i]);
...
template<>
void  cblas_sqr<double>(const int N, const double *X, double *Y) {
#pragma omp parallel num_threads(4)
      #pragma omp for
  for(int i = 0; i < N; ++i)
    Y[i] = sqrt(X[i]);
...
template<>
void  cblas_sqr<float>(const int N, const float *X, float *Y) {
#pragma omp parallel num_threads(4)
      #pragma omp for
  for(int i = 0; i < N; ++i)
    Y[i] = sqrt(X[i]);
...
template<>
void  cblas_pow<double>(const int N, const double *X, const double alpha,  double *Y) {
#pragma omp parallel num_threads(4)
      #pragma omp for
  for(int i = 0; i < N; ++i)
    Y[i] = pow(X[i], alpha);
...
template<>
void  cblas_pow<float>(const int N, const float *X, const float alpha,  float *Y) {
#pragma omp parallel num_threads(4)
      #pragma omp for
  for(int i = 0; i < N; ++i)
    Y[i] = pow(X[i], alpha);
...
template<>
void  cblas_add_scalar(const int N, const float *X, const float alpha,  float *Y) {
#pragma omp parallel num_threads(4)
      #pragma omp for
  for(int i = 0; i < N; ++i)
    Y[i] = X[i] + alpha;
...
template<>
void  cblas_add_scalar(const int N, const double *X, const double alpha,  double *Y) {
#pragma omp parallel num_threads(4)
      #pragma omp for
  for(int i = 0; i < N; ++i)
    Y[i] = X[i] + alpha;
...
template<>
void  cblas_add<double>(const int N, const double *X, const double *Y,  double *Z) {
#pragma omp parallel num_threads(4)
      #pragma omp for
  for(int i = 0; i < N; ++i)
    Z[i] =  X[i] + Y[i];
...
template<>
void  cblas_div<double>(const int N, const double *X, const double *Y,  double *Z) {
#pragma omp parallel num_threads(4)
      #pragma omp for
  for(int i = 0; i < N; ++i)
    Z[i] =  X[i] / Y[i];
...
template <>
float cblas_max<float>(const int N, const float* X) {
  return sse2_smax(N, X);
}

template <>
double cblas_max<double>(const int N, const double* X) {
