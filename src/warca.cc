...
template <typename scalar_t> WARCA<scalar_t>::WARCA(scalar_t seed, bool quiet) {
  dim_ = 0;
  num_svs_ = 0;
  dim_svs_ = 0;
  rank_ = 0;
...
template <typename scalar_t> WARCA<scalar_t>::WARCA(scalar_t seed) {
  dim_ = 0;
  num_svs_ = 0;
  dim_svs_ = 0;
  rank_ = 0;
...
template <typename scalar_t> WARCA<scalar_t>::WARCA() {
  dim_ = 0;
  num_svs_ = 0;
  dim_svs_ = 0;
  rank_ = 0;
...
template <typename scalar_t> WARCA<scalar_t>::WARCA(bool quiet) {
  dim_ = 0;
  num_svs_ = 0;
  dim_svs_ = 0;
  rank_ = 0;
...
void WARCA<scalar_t>::load_model(const char *filename) {
  ...
  if (kernel_type_ != PRE_COMP && kernel_type_ != LIN) {
    if (num_svs_ == 0 || dim_svs_ == 0) {
      throw warca_error("Number and dimension of support vectors should be "
                        "greater than zero\n");
    }
...
template <typename scalar_t> WARCA<scalar_t>::WARCA(const char *filename) {
  weight_ = NULL;
  SVs_ = NULL;
  load_model(filename);
  rnd_ = new Prand(1);
...
void WARCA<scalar_t>::save_model(const char *filename) {
  ...
  for (uint32_t i = 0; i < rank_ * dim_ - 1; i++) {
    fout << weight_[i] << ",";
  }
  fout << weight_[rank_ * dim_ - 1] << std::endl;
  if (kernel_type_ != PRE_COMP && kernel_type_ != LIN) {
    if (SVs_ == NULL) {
      throw warca_error("SVs should not be NULL for kernel type other than "
                        "PRE_COMP and LIN\n");
    }
    for (uint32_t i = 0; i < num_svs_ * dim_svs_ - 1; i++) {
      fout << SVs_[i] << ",";
    }
...
template <typename scalar_t>
scalar_t WARCA<scalar_t>::l2dist(const scalar_t *x, const scalar_t *y,
                                 uint32_t d) {
...
template <typename scalar_t>
void WARCA<scalar_t>::compute_chi2rbf_kernel(const scalar_t *x_matrix,
                                             const scalar_t *y_matrix,
                                             scalar_t *kernel_matrix,
                                             uint32_t num_x, uint32_t num_y,
                                             uint32_t dim) {
  if (gamma_ == 0) {
    gamma_ = scalar_t(1) / scalar_t(dim);
  }
...
template <typename scalar_t>
void WARCA<scalar_t>::compute_polynomial_kernel(const scalar_t *x_matrix,
                                                const scalar_t *y_matrix,
                                                scalar_t *kernel_matrix,
                                                uint32_t num_x, uint32_t num_y,
                                                uint32_t dim) {
  cblas_gemm<scalar_t>(CblasNoTrans, CblasTrans, num_x, num_y, dim,
                       scalar_t(gamma_), x_matrix, y_matrix, scalar_t(0),
                       kernel_matrix);
  cblas_add_scalar<scalar_t>(num_x * num_y, kernel_matrix, coeff_,
                             kernel_matrix);
  cblas_pow<scalar_t>(num_x * num_y, kernel_matrix, scalar_t(degree_),
                      kernel_matrix);
}
...
void WARCA<scalar_t>::compute_polynomial_kernel(const scalar_t *x_matrix,
                                                scalar_t *kernel_matrix,
                                                uint32_t num_x, uint32_t dim) {
  cblas_gemm<scalar_t>(CblasNoTrans, CblasTrans, num_x, num_x, dim,
                       scalar_t(gamma_), x_matrix, x_matrix, scalar_t(0),
                       kernel_matrix);
  cblas_add_scalar<scalar_t>(num_x * num_x, kernel_matrix, coeff_,
                             kernel_matrix);
  cblas_pow<scalar_t>(num_x * num_x, kernel_matrix, scalar_t(degree_),
                      kernel_matrix);
}

template <typename scalar_t>
void WARCA<scalar_t>::compute_rbf_kernel(const scalar_t *x_matrix,
                                         const scalar_t *y_matrix,
                                         scalar_t *kernel_matrix,
                                         uint32_t num_x, uint32_t num_y,
                                         uint32_t dim) {
...
template <typename scalar_t>
scalar_t WARCA<scalar_t>::warp_gradient_dual(const scalar_t *M,
                                             const uint32_t *labels,
                                             scalar_t *G, uint32_t batch_size,
                                             const uint32_t max_sampling,
                                             SimilarPairs *pairs) {
  ...
  for (uint32_t l = 0; l < batch_size; l++) {
    ...
    scalar_t sqrt_ik = scalar_t(0), sqrt_jk = scalar_t(0),
             loss_ij = scalar_t(0), loss_ji = scalar_t(0);
    ...
    do {
      ...
      if (loss_ij > scalar_t(0) || loss_ji > scalar_t(0)) {
        ...
        scalar_t warp_coeff =
            scalar_t(1) +
            scalar_t(log(scalar_t(max_sampling) / scalar_t(times_sampled)));
        if (loss_ij > scalar_t(0)) {
          loss_ij = scalar_t(1);
        }
        if (loss_ji > scalar_t(0)) {
          loss_ji = scalar_t(1);
        }
        scalar_t *g_k = G + dim_ * k;
        g_i[i] +=
            warp_coeff * ((loss_ij + loss_ji) / sqrt_ij - loss_ij / sqrt_ik);
        g_i[j] -= warp_coeff * ((loss_ij + loss_ji) / sqrt_ij);
        g_i[k] += warp_coeff * (loss_ij / sqrt_ik);
        g_j[j] +=
            warp_coeff * ((loss_ji + loss_ij) / sqrt_ij - loss_ji / sqrt_jk);
...
template <typename scalar_t>
void WARCA<scalar_t>::train_adam_dual(const scalar_t *kernel_matrix,
                                      const uint32_t *labels, scalar_t lambda,
                                      scalar_t eta, uint32_t max_iter,
                                      uint32_t batch_size,
  ...
  uint32_t iter = 0;
  while (iter < max_iter) {
    cblas_gemm<scalar_t>(CblasNoTrans, CblasTrans, dim_, rank_, dim_,
                         scalar_t(1), kernel_matrix, weight_, scalar_t(0), M);
    loss = warp_gradient_dual(M, labels, G, batch_size, max_sampling, pairs);
    cblas_gemm<scalar_t>(CblasTrans, CblasNoTrans, rank_, dim_, dim_,
                         scalar_t(2.0 / batch_size), M, G, scalar_t(0), G_sqr);
    cblas_gemm<scalar_t>(CblasTrans, CblasTrans, rank_, rank_, dim_,
                         (scalar_t)1, M, weight_, scalar_t(0), R);
    for (uint32_t i = 0; i < rank_; i++) {
      R[i * rank_ + i] -= scalar_t(1);
    }
    cblas_gemm<scalar_t>(CblasNoTrans, CblasNoTrans, rank_, dim_, rank_,
                         lambda * scalar_t(2), R, weight_, (scalar_t)1, G_sqr);
    ...
    cblas_axpby<scalar_t>(rtd, scalar_t(1) / (scalar_t(1) - beta2), V_adam,
                          scalar_t(0), G_sqr);
...
template <typename scalar_t>
void WARCA<scalar_t>::train(const scalar_t *data, const uint32_t *labels,
                            uint32_t num_data, uint32_t dim_data, uint32_t rank,
                            scalar_t lambda, scalar_t eta, uint32_t max_iter,
                            uint32_t batch_size, uint32_t max_sampling,
                            KernelType kernel_type, scalar_t gamma,
                            uint32_t degree, scalar_t coeff) {
  if (weight_ != NULL) {
    delete[] weight_;
  }
  ...
  if (kernel_type_ == PRE_COMP) {
    dim_ = num_data;
    weight_ = new scalar_t[rank_ * dim_];
    train_adam_dual(data, labels, lambda, eta, max_iter, batch_size,
                    max_sampling);
    return;
  }
  if (kernel_type_ != LIN) {
  ...
  case LIN:
    ...
    break;
  case LINK:
    cblas_gemm<scalar_t>(CblasNoTrans, CblasTrans, num_data, num_data, dim_data,
                         scalar_t(1), data, data, scalar_t(0), kernel_matrix);
    break;
  case POLYNOMIAL:
  ...
  if (kernel_type != LIN) {
    train_adam_dual(kernel_matrix, labels, lambda, eta, max_iter, batch_size,
                    max_sampling);
  } else {
    throw warca_error("Currently not supported,  it will be comming soon\n");
  }
...
template <typename scalar_t>
void WARCA<scalar_t>::predict(const scalar_t *data, scalar_t *embedding,
                              uint32_t num_data, uint32_t dim_data) {
  scalar_t *kernel_matrix = NULL;
  switch (kernel_type_) {
  case PRE_COMP:
    if (dim_data != dim_) {
    ...
    cblas_gemm<scalar_t>(CblasNoTrans, CblasTrans, num_data, rank_, dim_,
                         scalar_t(1), data, weight_, scalar_t(0), embedding);
  case LINK:
    kernel_matrix = new scalar_t[num_data * num_svs_];
    if (dim_data != dim_svs_) {
      throw warca_error("The input size doesn't match with SVs\n");
    }
    cblas_gemm<scalar_t>(CblasNoTrans, CblasTrans, num_data, num_svs_, dim_data,
                         scalar_t(1), data, SVs_, scalar_t(0), kernel_matrix);
    cblas_gemm<scalar_t>(CblasNoTrans, CblasTrans, num_data, rank_, dim_,
                         scalar_t(1), kernel_matrix, weight_, scalar_t(0),
                         embedding);
    delete[] kernel_matrix;
    break;
  case POLYNOMIAL:
    ...
    kernel_matrix = new scalar_t[num_data * num_svs_];
    compute_polynomial_kernel(data, SVs_, kernel_matrix, num_data, num_svs_,
                              dim_data);
    cblas_gemm<scalar_t>(CblasNoTrans, CblasTrans, num_data, rank_, dim_,
                         scalar_t(1), kernel_matrix, weight_, scalar_t(0),
                         embedding);
  ...
  case CHI2RBF:
    ...
    kernel_matrix = new scalar_t[num_data * num_svs_];
    compute_chi2rbf_kernel(data, SVs_, kernel_matrix, num_data, num_svs_,
                           dim_data);
    cblas_gemm<scalar_t>(CblasNoTrans, CblasTrans, num_data, rank_, dim_,
                         scalar_t(1), kernel_matrix, weight_, scalar_t(0),
                         embedding);
