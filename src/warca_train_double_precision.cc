...
int main(int argc, char* argv[]) {
  ...
  WARCATrainParameters<double> param;
  parse_command_line<double>(argc, argv, feature_filename, label_filename,  model_filename, param);
  uint32_t num_data, dim_data;
  double* data = NULL;
  uint32_t* labels = NULL;
  read_problem<double>(feature_filename, label_filename, &data, &labels, num_data, dim_data);
  WARCA<double> warca_trainer(param.seed, param.quiet);
  warca_trainer.train(data, labels, num_data, dim_data, param.rank, param.lambda, param.eta, param.max_iter, param.batch_size, param.max_sampling, param.kernel_type, param.gamma, param.degree, param.coeff);
